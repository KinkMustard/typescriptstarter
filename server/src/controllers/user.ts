...
import { IAuthToken, UserModel, UserType } from "../models/User";
...
export let postLogin = (req: Request, res: Response, next: NextFunction) => {
  ...
  passport.authenticate(
    "local",
    (err: Error, user: UserType, info: IVerifyOptions) => {
      if (err) {
        return next(err);
      }
      if (!user) {
...
export let postUpdateProfile = (
  ...
  UserModel.findById(req.user.id, (err, user: UserType) => {
    if (err) {
      return next(err);
    }
...
export let getOauthUnlink = (
  ...
  UserModel.findById(req.user.id, (err, user: any) => {
    ...
    user.tokens = user.tokens.filter(
      (token: IAuthToken) => token.kind !== provider
    );
    user.save((err: WriteError) => {
      if (err) {
        return next(err);
...
export let postForgot = (req: Request, res: Response, next: NextFunction) => {
  ...
  async.waterfall(
    [
      ...
      function setRandomToken(token: IAuthToken, done: Function) {
